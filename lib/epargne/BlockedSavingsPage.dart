import 'package:epargne_plus/epargne/BlockeddoSaving.dart';
import 'package:epargne_plus/service/api_service.dart';
import 'package:flutter/material.dart';
import '../components/custom_colors.dart';
class BloquePage extends StatefulWidget {
  final String amount;
  ...
  @override
  _BloquePageState createState() => _BloquePageState();

}
class _BloquePageState extends State<BloquePage>{
  ...
  Future<void> _fetchblockSavings() async {
  try {
    ...
    if (response['success'] == true) {
      ...
      if (data is List<dynamic>) {
        setState(() {
          transactions = data
              .where((t) =>
                  t['savingType']?.toString().toLowerCase() == 'blocked')
              .toList();
        });
      } else {
        setState(() {
  ...
  Future<void> _fetchSimpleSavings() async {
    try {
      ...
      if (response['success'] == true) {
        ...
        if (data is List<dynamic>) {
          setState(() {
            transactions = data.where((t) =>t['savingType']?.toString().toLowerCase() == 'blocked')
                .toList();
          });
        } else {
          setState(() {
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () => Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => BlockeddoSaving(
